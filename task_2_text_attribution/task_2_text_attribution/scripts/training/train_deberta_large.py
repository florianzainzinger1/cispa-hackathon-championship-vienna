...
# --- CONFIG: DeBERTa-v3-LARGE (OFFLINE) ---
SCRATCH_DIR = "/p/scratch/training2562/zainzinger1"
MODEL_NAME = os.path.join(SCRATCH_DIR, "models/deberta-v3-large")  # Lokales Modell!
OUTPUT_DIR = os.path.join(SCRATCH_DIR, "model_deberta_large_trained")

MAX_LEN = 512 
BATCH_SIZE = 8  # Kleiner wegen größerem Modell
EPOCHS = 5
LR = 1e-5  # Niedriger für großes Modell
...
def main():
    print(f"=== STARTE TRAINING: DeBERTa-v3-LARGE ===")
    print(f"Modell: {MODEL_NAME}")
    print(f"Output: {OUTPUT_DIR}")
    
    if not os.path.exists("participants_train.csv"):
        raise FileNotFoundError("CSV fehlt!")
    ...
    # Tokenizer - direkt von HuggingFace (muss auf Login-Node laufen!)
    ...
    print("Tokenisiere...")
    ds_train = ds_train.map(token_fn, batched=True)
    ds_val = ds_val.map(token_fn, batched=True)
    ...
    args = TrainingArguments(
        ...
        fp16=True,
        bf16=False,  # A100 supports bf16, but fp16 is more stable
        report_to="none",
        save_total_limit=1,
